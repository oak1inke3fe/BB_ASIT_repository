...
for root, dirnames, filenames in os.walk(filepath): #this is for looping through files that are in a folder inside another folder
    for filename in natsort.natsorted(filenames):
        ...
        if filename.startswith("mNode_Port1"):
            ...
            regex = r"(\d{3}.{1}\d{3,}).{1}(\d{1,2}.{1}\d{1,2}).{1}(\d{1,2}.{1}\d{1,2}).{1}(\d{1,2}.{1}\d{1,2}).{1}(\d{1,2}.{1}\d{1,2}).{1}(\d{1,2}.{1}\d{1,2}).{1}(\d{3,4}).{1}(\d{1,2}.{1}\d{1,2}).{1}(\d{1,2}.{1}\d{1,}).{1}(.{1}\d{1,3}.{1}\d{1,}).{1}(.{0,1}\d{1,2}.{1}\d{2}).{1}(\d{1}.{1}\d{1,3}).{1}(.{1,}).{1}(.{1,}).{1}(.{1,})"
            textfile = open(file, 'r')
            matches = []
            # reg = re.compile(regex)
            for line in textfile:
